...
async function Make_Html(data){
    ...
    if(!fn_IsEmpty(arr)){
        $.each(arr, function (index, el) {
            html += `
                <tr>
                    <td class="ltTbody">${el.code}</td>
                    <td class="ltTbody">${el.sname}</td>
                    <td class="ltTbody">${el.gubun}</td>
                    <td class="ltTbody">${el.loss}%</td>  
                    <td class="ltTbody">${el.cnt}건</td>  
                </tr>
            `;
        });
...
async function Data_Load(data){
    ...
    try {
        ...
        let url = APIURL + '/Load_Instructions_Info3';
        let result = await Load_API_Auth(url,dataarr);
        if (result.get('status') == 'NoLogin') {
            go_login();
        }else if(result.get('status') == 'ok') {
            let data = result.get('data');
            r_arr = (data && data.list) ? data.list : [];
        }else{
